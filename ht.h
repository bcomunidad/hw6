...
{
    Hash2 h2_;              /// h2(k)
    HASH_INDEX_T dhstep_;   /// Stepsize to use for double hash probing (some prime moduli less than the current hash table size)
...
{
    size_t loc = size()*100;
    size_t capacity = CAPACITIES[mIndex_]*40;
    if(loc >= capacity) {
        resize();
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    if(mIndex_ >= sizeof(CAPACITIES)/sizeof(CAPACITIES[0])) {
        throw std::logic_error("No more capacities available");
    }
    std::vector<HashItem*> oldTable = table_;
    table_.clear();
    table_.resize(CAPACITIES[mIndex_], nullptr);
    for(size_t i = 0; i < oldTable.size(); i++) {
        HashItem* item = oldTable[i];
